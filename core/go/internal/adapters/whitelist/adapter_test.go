...
func TestConfigure(t *testing.T) {
	cfg := provider.ProviderConfig{
		...
		Settings:    map[string]any{"display_name": "TestNode", "tunnel_mode": "vp8", "reliable": true},
	}
	...
	if !a.sessCfg.Reliable {
		t.Error("Reliable = false, want true so the upstream MultiTrack KCP layer can be selected")
	}
	if a.done == nil {
...
func TestStartForwardsReliableKCPAndKeepsDataTunnelContract(t *testing.T) {
	a := configuredProvider(t, provider.ProviderConfig{
		Credentials: map[string]string{"room_token": "room-token"},
		Endpoints:   map[string]string{"server_url": "wss://livekit.example.com", "room_id": "room-42"},
		Settings:    map[string]any{"tunnel_mode": "video", "reliable": true},
	})

	oldNewSession := newWBStreamSession
	oldStart := startWBStreamSession
	defer func() {
		newWBStreamSession = oldNewSession
		startWBStreamSession = oldStart
	}()

	var upstreamConfig wbstream.SessionConfig
	var upstreamSession *wbstream.Session
	newWBStreamSession = func(cfg wbstream.SessionConfig) *wbstream.Session {
		upstreamConfig = cfg
		upstreamSession = wbstream.NewSession(cfg)
		return upstreamSession
	}
	startWBStreamSession = func(*wbstream.Session) error { return nil }
	...
	if !upstreamConfig.Reliable || upstreamConfig.TunnelMode != "video" {
		t.Fatalf("upstream session config = %+v, want reliable video mode", upstreamConfig)
	}
	if upstreamSession == nil || upstreamSession.OnConnected == nil {
		t.Fatal("upstream session must receive the adapter DataTunnel callback")
	}

	mt := &mockTunnel{}
	upstreamSession.OnConnected(mt)
	if a.DataTunnel() != tunnel.DataTunnel(mt) {
		t.Fatal("connected reliable session lost the common DataTunnel contract")
	}
	a.SetOnData(func([]byte) {})
	a.SetOnClose(func() {})
	if mt.onDataFn == nil || mt.onCloseFn == nil {
		t.Fatal("reliable session did not preserve DataTunnel callbacks")
	}
}
...
func TestGetSchema(t *testing.T) {
	...
	if len(schema.Fields) != 7 {
		t.Errorf("schema.Fields count = %d, want 7", len(schema.Fields))
	}
	hasReliable := false
	for _, f := range schema.Fields {
		...
		if f.Name == "reliable" && f.Type == "boolean" && !f.Required {
			hasReliable = true
		}
	}
	if !hasReliable {
		t.Fatal("schema must expose optional boolean reliable MultiTrack KCP mode")
	}
}
