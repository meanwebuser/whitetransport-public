...
func TestGetSchema(t *testing.T) {
	...
	if schema.Name != "OK Messages" {
		t.Errorf("schema.Name = %q", schema.Name)
	}
	if len(schema.Fields) != 2 {
		t.Errorf("schema.Fields count = %d, want 2", len(schema.Fields))
	}
...
func TestConfigureValid(t *testing.T) {
	a := &Provider{}
	err := a.Configure(provider.ProviderConfig{
		Credentials: map[string]string{"token": "test-ok-token-123"},
		Endpoints:   map[string]string{"api_url": "https://api.ok.ru/graph", "chat_id": "chat-1"},
		Settings:    map[string]any{},
	})
	if err != nil {
...
func TestHealthAndMetrics(t *testing.T) {
	...
	if limits.MaxRatePerMinute != 90 {
		t.Errorf("MaxRatePerMinute = %d, want 90", limits.MaxRatePerMinute)
	}
	if limits.MaxDailyBytes != 844_000_000 {
	...
	a.UpdateMetrics(provider.Metrics{SentBytes: 300, Errors: 1})
	m = a.GetMetrics()
	if m.SentBytes != 300 || m.Errors != 1 {
