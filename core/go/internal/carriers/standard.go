...
const (
	...
	CarrierMailIMAPSMTP = "mail.imap_smtp"
...
//
//	vk.messages, ok.messages, vk.docs.*, ok.docs.*,
//	yandex.disk.files, ssh.tcp, singbox.vless
//
// Bridged descriptor (created via ProviderCarrier + adapter registry):
//
//	wbstream.vp8
//
// vk.photos and ok.photos remain catalog obligations without a Go constructor;
// CapabilityInventory keeps those and non-descriptor registry/media rows
// explicit until their vertical slices are implemented and proven.
func StandardDescriptors() []Descriptor {
	return []Descriptor{
		...
		},
		{
			...
			Notes:          "Append-only Git branch carrier; local git:// proof does not imply authenticated provider readiness.",
		},
		{
			...
			Notes:          "TLS-authenticated SMTP submission plus retained IMAP UID polling; message-body carrier for control and low-throughput egress.",
		},
		{
			ID:             CarrierSSHTCP,
			Provider:       "ssh",
