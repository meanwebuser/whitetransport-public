...
func TestCarrierTunnelConnExplicitCloseSuppressesLateProviderFailure(t *testing.T) {
	left, right := net.Pipe()
	defer right.Close()
	failure := &carrierFailureState{}
	conn := &carrierTunnelConn{Conn: left, failure: failure}
	if err := conn.Close(); err != nil {
		t.Fatalf("close tunnel conn: %v", err)
	}
	failure.set(errors.New("provider poll stopped after close"))
	if cause := conn.FailureCause(); cause != nil {
		t.Fatalf("explicit close retained a non-causal provider failure: %v", cause)
	}
}
