...
func TestReporterSendUsesTokenReference(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		if got := request.Header.Get("Authorization"); got != "Bearer fixture-telemetry-token" {
			t.Fatalf("Authorization = %q, want TokenStore reference", got)
		}
	...
	reporter := Reporter{
		cfg: config.AdminReporterConfig{AdminURL: server.URL, TokenRef: "mac-manual-telemetry"},
		node: config.Config{
			NodeID: "mac-manual-client",
			TokenStore: &config.TokenStoreConfig{Tokens: []config.TokenEntry{{
				ID: "mac-manual-telemetry", Status: "active", Value: "fixture-telemetry-token",
			}}},
		},
		status: fakeStatusProvider{},
		client: server.Client(),
	}
	if err := reporter.send(context.Background(), "/telemetry"); err != nil {
		t.Fatalf("send: %v", err)
	}
}
