...
func Start(ctx context.Context, cfg config.Config, tokenStore *tokens.Store) (_ *Transport, returnErr error) {
	...
	bindings, err := runtime.BuildCarrierBindingsWithRegistryAndTokens(cfg, reg, tokenStore)
	if err != nil {
		cancel()
	...
	if strings.TrimSpace(cfg.SocksListen) != "" {
		...
		srv := &wtproxy.Server{
			...
			EgressDialer:       control.DialEgress,
			EgressPacketDialer: control.OpenPacketEgress,
			Logf:               log.Printf,
		}
		t.proxy = srv
...
func (t *Transport) Connect(ctx context.Context, nodeID string) (runtime.StatusView, error) {
	if t.control == nil {
		return runtime.StatusView{}, errors.New("transport not started")
	}
