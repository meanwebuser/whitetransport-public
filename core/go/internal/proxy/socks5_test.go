...
func TestServerUDPAssociateFallsBackToDNSOverCarrierStream(t *testing.T) {
	dnsListener, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		t.Fatalf("listen fake DNS TCP: %v", err)
	}
	...
	go func() {
		...
		defer conn.Close()
		var size [2]byte
		if _, acceptErr = io.ReadFull(conn, size[:]); acceptErr != nil {
			dnsDone <- acceptErr
			return
		}
		query := make([]byte, binary.BigEndian.Uint16(size[:]))
		if _, acceptErr = io.ReadFull(conn, query); acceptErr != nil {
			dnsDone <- acceptErr
			return
		}
	...
	server := Server{
		...
		EgressDialer: func(ctx context.Context, targetAddr string) (net.Conn, string, error) {
			if targetAddr != dnsStreamResolver {
				return nil, "", fmt.Errorf("unexpected DNS target %s", targetAddr)
			}
			conn, dialErr := (&net.Dialer{}).DialContext(ctx, "tcp", dnsListener.Addr().String())
			return conn, "fake-carrier-stream", dialErr
		},
		EgressPacketDialer: func(context.Context, session.PacketMetadata) (net.PacketConn, string, error) {
	...
	control, err := net.DialTimeout("tcp", server.Addr(), 5*time.Second)
	if err != nil {
		t.Fatalf("dial SOCKS control: %v", err)
	}
	defer control.Close()
	...
	request := []byte{socksVersion5, socksCmdUDPAssociate, 0x00, socksAtypIPv4, 127, 0, 0, 1}
	...
	if _, err := io.ReadFull(control, reply); err != nil || reply[1] != 0 {
		t.Fatalf("read UDP ASSOCIATE reply=%v err=%v", reply, err)
	}
	relayAddr := net.JoinHostPort(net.IP(reply[4:8]).String(), strconv.Itoa(int(binary.BigEndian.Uint16(reply[8:10]))))
	query := []byte{0x12, 0x34, 0x01, 0x00, 0x00, 0x01}
	frame := buildSOCKSUDPDatagram("192.168.2.1", 53, query)
	if _, err := udpClient.WriteTo(frame, mustUDPAddr(t, relayAddr)); err != nil {
	...
	if destination != "192.168.2.1:53" || len(payload) < 4 || payload[2]&0x80 == 0 {
		t.Fatalf("DNS response destination=%q payload=%x", destination, payload)
	}
	if err := <-dnsDone; err != nil {
		t.Fatalf("fake DNS TCP: %v", err)
	}

	cancel()
	if err := <-errCh; err != nil {
		t.Fatalf("SOCKS shutdown: %v", err)
	}
}
