...
// RecordConstructed registers a carrier that passed construction and is
// available for runtime health checks.
func (h *CarrierHealth) RecordConstructed(carrierID string) {
	h.mu.Lock()
	defer h.mu.Unlock()
	e := h.getEntry(carrierID)
	e.lifecycleState = "constructed"
	e.failureStage = ""
	e.errorCode = ""
	e.retryable = false
	e.resourceGroup = ""
...
// RecordInitializationFailure registers a carrier that could not enter the
// runtime. The stable code is safe for API consumers; the underlying error is
// deliberately not retained here.
func (h *CarrierHealth) RecordInitializationFailure(carrierID, stage, code string, retryable bool, resourceGroup string) {
	h.mu.Lock()
	defer h.mu.Unlock()
	e := h.getEntry(carrierID)
...
// RecordRuntimeFailure marks a constructed binding unavailable after concrete
// I/O failure without exposing provider error text.
func (h *CarrierHealth) RecordRuntimeFailure(carrierID string) {
	h.RecordInitializationFailure(carrierID, "runtime", "io_failure", true, carrierID)
}
...
func (h *CarrierHealth) Snapshot() map[string]CarrierSnapshot {
	...
	for id, e := range h.entries {
		...
		out[id] = CarrierSnapshot{
			CarrierID:      id,
			Healthy:        e.lifecycleState != "degraded" && (e.readFailures == 0 || e.readSuccesses > e.readFailures),
...
func (h *CarrierHealth) Metrics(carrierID string) carriers.Metrics {
	...
	if ok {
		transportHealthy = e.lifecycleState != "degraded" && (e.readFailures == 0 || e.readSuccesses > e.readFailures)
	}
