...
// TokenValueFromStore resolves an embedded telemetry credential only after
// explicit inline and environment configuration have been considered.
func (c AdminReporterConfig) TokenValueFromStore(store TokenStoreConfig) string {
	if token := c.TokenValue(); token != "" {
		return token
	}
	ref := strings.TrimSpace(c.TokenRef)
	if ref == "" {
		return ""
	}
	for _, token := range store.Tokens {
		if token.ID == ref && strings.TrimSpace(string(token.Status)) == "active" {
...
type CarrierConfig struct {
	...
	MailIMAPSMTP    *MailIMAPSMTPConfig    `json:"mail_imap_smtp,omitempty"`
	Telemost        *TelemostConfig        `json:"telemost,omitempty"`
	Dion            *DionConfig            `json:"dion,omitempty"`
	VKCall          *VKCallConfig          `json:"vk_call,omitempty"`
	TokenRef        string                 `json:"token_ref,omitempty"`
...
// GitRepositoryConfig configures an append-only Git envelope carrier. This
// local stage supports unauthenticated file/git protocol remotes only.
...
// MailIMAPSMTPConfig contains only non-secret mailbox connection metadata.
// SMTP and IMAP credentials are resolved from an exact mail/imap_smtp
// TokenStore binding keyed by AccountID.
...
// VKCallConfig contains non-secret VK Call video tunnel settings. Browser
// session cookies are resolved at runtime from the TokenStore binding for
// platform vk / connection_type calls, never from this config block.
type VKCallConfig struct {
	JoinLink        string `json:"join_link,omitempty"`
	...
	VP8Batch        int    `json:"vp8_batch,omitempty"`
...
func (c Config) Validate() error {
	...
	for _, carrierConfig := range c.CarrierConfigs {
		if role := strings.TrimSpace(carrierConfig.Role); role != "" && !validCarrierRole(role) {
			return fmt.Errorf("carrier config %s has invalid role %q", carrierConfig.ID, role)
		}
...
func validCarrierRole(role string) bool {
	switch role {
	case "discovery", "node-client", "logs", "admin", "node", "client", "egress", "flex", "bulk":
		return true
	default:
		return false
	}
...
// Adapter-registry carriers (wbstream, telemost, dion, vkcall) are resolved at
