...
func (c *ControlPlane) systemVPNBindingInputsLocked(active *activeSession) []SystemVPNBindingInput {
	...
	if active != nil && len(active.EgressEndpoints) > 0 {
		// Prefer the exact session endpoint carrier when the configured egress
		// list contains a matching binding; this keeps the dependency set tied
		// to the active/failover carriers rather than a stale config alias.
		for _, endpoint := range active.EgressEndpoints {
			matched := false
			for _, ref := range c.egress {
				if ref.ID == endpoint.Carrier || ref.Descriptor.ID == endpoint.Carrier || ref.Binding.Endpoint.Carrier == endpoint.Carrier {
					matched = true
					break
				}
			}
			...
			// Session answers may advertise transport-specific candidates (for
			// example ssh.tcp) that have no client config-bound origin. They are
			// not authoritative system-route dependencies; configured carriers
			// remain the failover set used for profile construction.
		}
	}
