...
// global outage. The node advertises over the healthy carrier, the client
// first attempts the failed contact, and the node must deliver its answer via
// the healthy reply endpoint.
func TestControlPlaneConnectsWhenOneControlCarrierCannotWrite(t *testing.T) {
	...
	// Map iteration must not decide the resilience contract: exercise the
	// failed path first, then require the healthy reply path to take over.
	...
	if failedCarrier.writeCount(session.PayloadSessionAnswer) != 1 {
		t.Fatalf("expected one failed session.answer write, got %d", failedCarrier.writeCount(session.PayloadSessionAnswer))
	}
