...
type ControlPlane struct {
	...
	clientIntentCtx          context.Context
	clientIntentCancel       context.CancelFunc
	connectionIntentRevision uint64
	connectionRetryAt        time.Time
	connectionRetryDelay     time.Duration
	clientEndpointPinned     bool
	connectionLivenessFailed bool
	connectionCarrierFailed  bool
	controlStartedAt         time.Time
	abandonedSessions        map[string]*activeSession
...
func (c *ControlPlane) Start(ctx context.Context) {
	c.mu.Lock()
	c.controlStartedAt = time.Now().UTC()
	c.mu.Unlock()
...
func (c *ControlPlane) handleControlEnvelope(ctx context.Context, envelope fabric.Envelope, ref carrierRef) {
	...
	case session.PayloadSessionOffer:
		...
		if c.offerPredatesNodeStart(envelope) {
			return
		}
...
func (c *ControlPlane) Connect(ctx context.Context, nodeID string) (StatusView, error) {
	c.mu.RLock()
	revision := c.connectionIntentRevision
	c.mu.RUnlock()
	if err := c.lockNodeRecovery(ctx); err != nil {
		return c.Status(), err
	}
	defer c.nodeAutoHealMu.Unlock()
	c.mu.Lock()
	if revision != c.connectionIntentRevision {
		c.mu.Unlock()
		return c.Status(), context.Canceled
	}
	if c.clientIntentCancel != nil {
		c.clientIntentCancel()
	}
	c.clientIntentCtx, c.clientIntentCancel = context.WithCancel(context.Background())
	intentCtx := c.clientIntentCtx
	c.connectionRetryAt = time.Time{}
	c.connectionRetryDelay = 0
	c.clientEndpointPinned = false
	c.mu.Unlock()
	attemptCtx, cancelAttempt := context.WithCancel(ctx)
	stopIntentCancellation := context.AfterFunc(intentCtx, cancelAttempt)
	defer stopIntentCancellation()
	defer cancelAttempt()
	ctx = attemptCtx
	c.mu.RLock()
...
func (c *ControlPlane) connectWithFailoverExcluding(ctx context.Context, nodeID string, excludedNodeID string) (StatusView, error) {
	...
	var lastErr error
	failedAuthBindings := make(map[string]bool)
	for _, nodeView := range orderedNodes {
		...
		for _, contactInfo := range contacts {
			...
			_, bindingKey := c.findBindingByCarrier(contactInfo.endpoint.Carrier)
			if failedAuthBindings[bindingKey] {
				continue
			}
			...
			// Rejection belongs to this node; authentication belongs to this
			// credential binding. Neither invalidates independent candidates.
			if strings.Contains(err.Error(), "node rejected offer") {
				break
			}
			if isNonRetriableError(err) {
				failedAuthBindings[bindingKey] = true
			}
		}
	}
...
func (c *ControlPlane) connectOnceWithContact(ctx context.Context, nodeID string, contact carriers.Endpoint, binding policy.CarrierBinding) (StatusView, error) {
	if err := c.releaseAbandonedSession(ctx, nodeID, contact, binding); err != nil {
		return c.Status(), fmt.Errorf("release abandoned session on node %s: %w", nodeID, err)
	}
...
waitForAnswer:
	...
	case answer := <-answerCh:
		...
		if answer.JoinedClientRoom && c.clientRoomEndpoint != "" {
			...
			c.connectionLivenessFailed = false
			c.connectionCarrierFailed = false
			...
			go c.monitorSessionLiveness(c.connectionLifetimeContext(ctx), answer)
...
func (c *ControlPlane) DialEgress(ctx context.Context, targetAddr string) (net.Conn, string, error) {
	...
	if active == nil {
		// Node auto-heal clears the failed session before connecting its
		// replacement. A concurrent dial must wait for that replacement rather
		// than observing the intentional transient as a disconnected client.
		if err := c.lockNodeRecovery(ctx); err != nil {
...
// once through another node after a typed carrier failure. A dial racing that
// recovery also coalesces when its route is quarantined or the old active
// session has just been cleared. Healthy dials stay concurrent; manual dials
// never enter this replacement path.
func (c *ControlPlane) dialAutomaticEgressWithLiveness(ctx context.Context, expected *activeSession, targetAddr string) (net.Conn, string, error) {
	active := c.activeSessionSnapshot()
	if active == nil {
		// A sibling DialEgress may have started automatic reselection after this
		// call took its first snapshot. Wait for that one bounded transition and
		// retry against the replacement, if it succeeded.
		if err := c.lockNodeRecovery(ctx); err != nil {
			return nil, "carrier-session", err
		}
		c.nodeAutoHealMu.Unlock()
	...
	if err != nil && (session.IsCarrierFailure(err) || errors.Is(err, errNoCompatibleAutomaticEgressEndpoint) || errors.Is(err, errAutomaticEgressRouteQuarantined) || errors.Is(err, errNoActiveCarrierSession)) {
...
func (c *ControlPlane) reselectNodeAfterCarrierExhaustionLocked(ctx context.Context, failed *activeSession, targetAddr string) (net.Conn, string, error) {
	attemptCtx, cancelAttempt := context.WithCancel(ctx)
	stopIntentCancellation := context.AfterFunc(c.connectionLifetimeContext(ctx), cancelAttempt)
	defer stopIntentCancellation()
	defer cancelAttempt()
	ctx = attemptCtx
	c.mu.Lock()
	...
	routeStreams := c.egressRouteStreams
	c.active = nil
	c.rememberAbandonedSessionLocked(active)
	c.egressRouteStreams = nil
	c.state = statusStateDisconnected
...
// errNoCompatibleAutomaticEgressEndpoint means every automatic route was
// filtered or quarantined before a dial could start. In automatic mode this
// is equivalent to exhausted carrier routes and must coalesce on replacement.
var errNoCompatibleAutomaticEgressEndpoint = errors.New("no compatible egress endpoint in active session")

// errAutomaticEgressRouteQuarantined marks the narrow race where a route was
// eligible during scoring but a sibling dial quarantined it before beginDial.
// Automatic mode must coalesce on node replacement instead of returning the
// stale route error; manual selection never enters the automatic recovery path.
var errAutomaticEgressRouteQuarantined = errors.New("route quarantined")

// errNoActiveCarrierSession is also transient when an automatic dial took an
// old-session snapshot immediately before a sibling recovery cleared it. The
// automatic path waits for that coalesced replacement; manual/disconnected
// callers still receive the same error without starting recovery.
var errNoActiveCarrierSession = errors.New("no active carrier session")
...
func (c *ControlPlane) dialActiveSessionEgressWithRegistry(ctx context.Context, active *activeSession, tunnel session.CarrierTunnel, recovery *policy.EgressRecoveryTracker, routeStreams *routeStreamRegistry, targetAddr string) (net.Conn, string, error) {
	...
	if active == nil {
		return nil, "", errNoActiveCarrierSession
	}
	if tunnel == nil {
	...
	return nil, "carrier-session", errNoCompatibleAutomaticEgressEndpoint
}
	...
	for _, ep := range batch {
		go func(ep carriers.Endpoint) {
			...
			if routeStreams != nil {
				...
				if !eligible {
					select {
					case results <- dialResult{err: fmt.Errorf("%s/%s: %w", ep.Carrier, ep.ID, errAutomaticEgressRouteQuarantined)}:
					case <-done:
					}
					return
				}
...
func (c *ControlPlane) monitorSessionLiveness(ctx context.Context, answer session.Answer) {
	...
	for {
		...
		case <-ticker.C:
			...
			if active.SelectedEgressEndpointID != "" {
				// A manual diagnostic route owns its own foreground health proof.
				// The original control endpoint may intentionally be unavailable.
				continue
			}
			if c.requiresSynchronousNodeLivenessProbe(active) {
				probeCtx, cancel := context.WithTimeout(ctx, nodeLivenessProbeTimeout)
				err := c.probeActiveNodeLiveness(probeCtx, active)
				cancel()
				if err != nil {
					c.mu.Lock()
					if c.active != nil && c.active.SessionID == answer.SessionID && c.active.SelectedEgressEndpointID == "" {
						c.connectionLivenessFailed = true
						c.state = statusStateDegraded
			...
			if lc, ok := tunnel.(livenessChecker); ok {
				if !lc.IsAlive() {
					...
					if failCount >= 2 {
						...
						c.mu.Lock()
						if c.active != nil && c.active.SessionID == answer.SessionID && c.active.SelectedEgressEndpointID == "" {
							c.lastError = fmt.Sprintf("tunnel liveness check failed %d times", failCount)
							c.state = statusStateDegraded
							c.connectionLivenessFailed = true
							c.mu.Unlock()
							return
...
func (c *ControlPlane) storeAdvertisement(ad session.NodeAdvertisement) {
	...
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.clientIntentCtx != nil && c.active == nil {
		c.connectionRetryAt = time.Time{}
		c.connectionRetryDelay = 0
		c.reconnectAttempts = 0
	}
...
func (c *ControlPlane) statusLocked() StatusView {
	...
	visibleState := c.state
	if c.active == nil && c.clientIntentCtx != nil && c.clientIntentCtx.Err() == nil && !c.clientEndpointPinned {
		visibleState = statusStateReconnecting
	}
	status := StatusView{
		Role:              c.cfg.Role,
		State:             visibleState,
