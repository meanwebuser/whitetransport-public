...
func NewControlPlaneWithTokens(cfg config.Config, bindings map[string]policy.CarrierBinding, carrierPolicy policy.CarrierPolicy, tunnel session.CarrierTunnel, ts *tokens.Store) (*ControlPlane, error) {
	...
	for id, binding := range bindings {
		...
		// Role-aware classification: when channel bindings are enabled and
		// a binding carries an explicit role, classify by role instead of
		// descriptor capabilities. This is the runtime side of per-role
		// endpoints (vk.messages:discovery, vk.messages:node-client, etc.).
		if config.ChannelBindingsEnabled() && binding.Role != "" {
	...
	if cfg.Role == config.RoleNode && len(control.bootstrap) == 0 {
		return nil, errors.New("node role requires at least one bootstrap-capable carrier binding")
	}
	if cfg.Role == config.RoleClient && len(control.control) == 0 && len(bindings) > 0 {
		return nil, errors.New("client role requires at least one control-capable carrier binding")
	}
...
func (c *ControlPlane) ConfigurePostSessionControl(cfg config.AdminRelayConfig) error {
	if !cfg.Enabled {
		return errors.New("post-session HTTP control must be enabled")
	}
	adminURL := strings.TrimSpace(cfg.AdminURL)
	if adminURL == "" {
		return errors.New("post-session HTTP control requires admin URL")
	}
	parsedURL, err := url.ParseRequestURI(adminURL)
	if err != nil || parsedURL.Host == "" || (parsedURL.Scheme != "http" && parsedURL.Scheme != "https") {
		return errors.New("post-session HTTP control requires an absolute HTTP(S) admin URL")
	}
	...
	if !slices.Contains(channels, "control") {
		return errors.New("post-session HTTP control requires the control channel")
	}
...
func (c *ControlPlane) DialEgress(ctx context.Context, targetAddr string) (net.Conn, string, error) {
	...
	c.mu.RUnlock()
	conn, route, err := c.dialActiveSessionEgress(ctx, active, tunnel, targetAddr)
	if active != nil {
...
func (c *ControlPlane) OpenPacketEgress(ctx context.Context, metadata session.PacketMetadata) (net.PacketConn, string, error) {
	...
	for index, endpoint := range endpoints {
		...
		if err == nil {
			c.recordEgressResult(active.SessionID, nil)
...
func (c *ControlPlane) dialActiveSessionEgress(ctx context.Context, active *activeSession, tunnel session.CarrierTunnel, targetAddr string) (net.Conn, string, error) {
	...
	for len(remaining) > 0 {
		...
		conn, route, err := dialBatch(ctx, tunnel, batch, targetAddr, dialEgressTimeout)
		if err == nil {
...
// classifyByRole maps a channel binding role to the appropriate control
// plane slices. This is used when WT_CHANNEL_BINDINGS=1 and the binding
// carries an explicit role from the config channels block.
