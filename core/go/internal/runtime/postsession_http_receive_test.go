...
func TestPostSessionHTTPRelayIsPolledAndDispatchedByNode(t *testing.T) {
	...
	nodeControl.mu.RUnlock()
	if nodeStillBusy || ackCount.Load() != 1 {
		t.Fatalf("node did not complete relayed %s: busy=%v acknowledgements=%d", session.PayloadSessionRelease, nodeStillBusy, ackCount.Load())
	}
	if getCount.Load() == 0 {
