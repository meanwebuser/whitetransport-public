...
func (c *ControlPlane) startEgressRecoveryLoop(ctx context.Context) {
	...
	go func() {
		...
		defer ticker.Stop()
		nextEndpointProbe := time.Now().Add(egressRecoveryTickInterval)
		for {
			select {
			...
			case <-c.stopCh:
				return
			case <-ticker.C:
				c.recoverConnectionIntent()
				if !time.Now().Before(nextEndpointProbe) {
...
func (c *ControlPlane) recordQuarantinedEgressFailure(sessionID string, endpoint carriers.Endpoint, dialErr error) {
	...
	if session.IsCarrierFailure(dialErr) {
		c.connectionCarrierFailed = true
	}
	if c.egressRecovery != nil {
...
// is observed by the daemon intent loop as well as the next automatic dial.
// Exhausted automatic routes recover without waiting for another SOCKS request.
