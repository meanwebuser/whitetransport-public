...
def dion_token_store():
  return {
    "tokens": [{
      ...
      "status": "active",
      "parts": {
        "access_token": os.environ["WT_DION_ACCESS_TOKEN"],
        "refresh_token": os.environ["WT_DION_REFRESH_TOKEN"],
        "cookies_file": f"{repo}/secrets/production/dion/dion-cookies.json"
      }
    }],
    "bindings": [
      {"token_id": "dion-live", "platform": "dion", "connection_type": "video", "channel_id": "*", "role": "creator", "priority": 100, "enabled": True},
      {"token_id": "dion-live", "platform": "dion", "connection_type": "video", "channel_id": "*", "role": "joiner", "priority": 100, "enabled": True}
    ]
  }
...
client_cfg = {
  ...
  "carrier_configs": [{
    ...
    "id": "dion",
    "dion": {"event_id": "dion-local", "display_name": "DION Client", "role": "joiner"},
    "endpoint": {"id": "dion-egress", "address": "dion-local"}
  }],
  "upstream_proxy": {"url": "", "client_egress_only": True, "apply_to_carriers": False},
  "token_store": dion_token_store()
