...
node_cfg = {
  ...
  "carrier_configs": [{
    ...
    "id": "telemost",
    "telemost": {"cookie_env": "WT_YANDEX_COOKIE", "display_name": "Telemost Node", "role": "creator"},
...
client_cfg = {
  ...
  "carrier_configs": [{
    ...
    "id": "telemost",
    "telemost": {"display_name": "Telemost Client", "role": "joiner", "cookie_env": "WT_YANDEX_COOKIE"},
    "endpoint": {"id": "telemost-egress", "address": "telemost-local"}
  }],
  "upstream_proxy": {"url": "", "client_egress_only": True, "apply_to_carriers": False}
}
...
run_probe_until_success "Telemost raw data" "$PROBE" -mode echo -socks 127.0.0.1:1280 -payload telemost-raw-data -timeout 6s
run_probe_until_success "Telemost custom TCP packet" "$PROBE" -mode packet -socks 127.0.0.1:1280 -payload telemost-custom-tcp-packet -timeout 6s
run_probe_until_success "Telemost HTTP" "$PROBE" -mode http -socks 127.0.0.1:1280 -target 127.0.0.1:18080 -host local.test -timeout 6s
