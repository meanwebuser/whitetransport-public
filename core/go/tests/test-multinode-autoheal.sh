...
# automatic session reselection to node B, then a second SOCKS payload.
set -Eeuo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
GO_ROOT="$REPO_ROOT/core/go"
RUN_DIR="$(mktemp -d /tmp/wt-multinode-autoheal.XXXXXX)"
...
cleanup() {
    ...
    if [[ -n "$CLIENT_PID" ]] && kill -0 "$CLIENT_PID" 2>/dev/null; then
        client_ns curl --noproxy '*' -sf -X POST "http://127.0.0.1:$CLIENT_API/v1/session/disconnect" --data '' >/dev/null 2>&1 || true
    fi
...
PHASE=disconnect
client_ns curl --noproxy '*' -sf -X POST "http://127.0.0.1:$CLIENT_API/v1/session/disconnect" --data '' >/dev/null || fail 'disconnect failed'
SUCCEEDED=true
