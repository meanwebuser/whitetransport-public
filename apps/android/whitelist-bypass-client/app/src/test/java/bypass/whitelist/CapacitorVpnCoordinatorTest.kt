...
class CapacitorVpnCoordinatorTest {
    ...
    @Test
    fun `restart reconciliation preserves established tunnel while transport repairs then restores connected`() {
        val dependencies = FakeDependencies(transportRecovering = true, tunnelActive = true)
        val coordinator = coordinator(dependencies)

        coordinator.reconcileAfterRestart()

        assertEquals(CapacitorVpnLifecycle.CONNECTING, coordinator.status().lifecycle)
        assertFalse(coordinator.status().transportConnected)
        assertTrue(coordinator.status().systemVpnConnected)
        assertTrue(dependencies.events.isEmpty())

        dependencies.transportRecovering = false
        dependencies.transportConnected = true

        assertEquals(CapacitorVpnLifecycle.CONNECTED, coordinator.status().lifecycle)
        assertTrue(dependencies.events.isEmpty())

        dependencies.transportConnected = false
        dependencies.transportRecovering = true
        assertEquals(CapacitorVpnLifecycle.CONNECTING, coordinator.status().lifecycle)
        assertTrue(coordinator.status().systemVpnConnected)
        assertFalse(coordinator.status().transportConnected)
        assertTrue(dependencies.events.isEmpty())
    ...
    @Test
    fun `runtime recovery states exclude deliberate disconnect and stopped runtime`() {
        listOf("connecting", "reconnecting", "degraded").forEach {
            assertEquals(CapacitorTransportState.RECOVERING, capacitorTransportState(it))
        }
        assertEquals(CapacitorTransportState.CONNECTED, capacitorTransportState("connected"))
        listOf("disconnected", "stopped", "idle", "error", "").forEach {
            assertEquals(CapacitorTransportState.DISCONNECTED, capacitorTransportState(it))
        }
    }

    @Test
    fun `recovery completing after transport snapshot cannot tear down established tunnel`() {
        val dependencies = FakeDependencies(transportRecovering = true, tunnelActive = true)
        dependencies.onTransportSnapshot = {
            dependencies.transportRecovering = false
            dependencies.transportConnected = true
        }
        val coordinator = coordinator(dependencies)
        ...
        assertEquals(CapacitorVpnLifecycle.CONNECTING, outcome!!.getOrThrow().lifecycle)
        assertTrue(outcome!!.getOrThrow().systemVpnConnected)
        assertEquals(CapacitorVpnLifecycle.CONNECTED, coordinator.status().lifecycle)
        assertTrue(dependencies.events.isEmpty())
    }

    @Test
    fun `explicit disconnect cancels transport recovery and stops existing tunnel`() {
        val dependencies = FakeDependencies(transportRecovering = true, tunnelActive = true)
        val coordinator = coordinator(dependencies)
        coordinator.reconcileAfterRestart()
        dependencies.events.clear()
        ...
        coordinator.disconnect { outcome = it }

        assertTrue(outcome!!.isSuccess)
        assertEquals(listOf("stopTunnel", "disconnect"), dependencies.events)
        assertFalse(dependencies.transportRecovering)
        assertEquals(CapacitorVpnLifecycle.DISCONNECTED, coordinator.status().lifecycle)
    }

    @Test
    fun `recovery without established tunnel still cleans stale starting service`() {
        val dependencies = FakeDependencies(transportRecovering = true, tunnelActive = false, tunnelInProgress = true)
        val coordinator = coordinator(dependencies)
    ...
    private class FakeDependencies(
        ...
        var transportRecovering: Boolean = false,
        ...
        var onTransportSnapshot: (() -> Unit)? = null
        ...
        override fun transportState(): CapacitorTransportState {
            val state = when {
                transportConnected -> CapacitorTransportState.CONNECTED
                transportRecovering -> CapacitorTransportState.RECOVERING
                else -> CapacitorTransportState.DISCONNECTED
            }
            onTransportSnapshot?.invoke()
            return state
        }
