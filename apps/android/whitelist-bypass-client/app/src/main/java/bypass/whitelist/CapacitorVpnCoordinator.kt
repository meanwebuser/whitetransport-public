...
class CapacitorVpnCoordinator(
    ...
    fun status(): CapacitorVpnStatus = currentStatus
    ...
    fun statusJson(): JSONObject = currentStatus.toJson()
    ...
    private fun executeRestartReconciliation(reconciliationId: Long): CapacitorVpnStatus {
        ...
        val transport = dependencies.isTransportConnected()
...
private object SocksPayloadProbe {
    fun verify(host: String, port: Int, user: String, pass: String): Boolean {
        Socket().use { socket ->
            ...
            val target = "api.ipify.org".toByteArray(Charsets.US_ASCII)
            output.write(byteArrayOf(0x05, 0x01, 0x00, 0x03, target.size.toByte()))
            output.write(target)
            output.write(byteArrayOf(0x00, 0x50))
            output.flush()
            ...
            input.skipFully(2)
            output.write("GET /?format=text HTTP/1.1\r\nHost: api.ipify.org\r\nConnection: close\r\n\r\n".toByteArray(Charsets.US_ASCII))
            output.flush()
            val body = input.bufferedReader(Charsets.US_ASCII).readText().substringAfter("\r\n\r\n", "").trim()
