...
object GoRuntimeController {
    ...
    fun ensureStarted(context: Context, configJson: String? = null): JSONObject {
        val cfg = configJson?.trim()?.takeIf { it.isNotEmpty() }
            ?.let { RuntimeConfigStore.resolveConfigJson(context, it) }
            ?: lastConfigJson
            ?: RuntimeConfigStore.resolveConfigJson(context)
        if (!isStarted()) {
            val startedWithRoomSession = LocalUserCredentialStore.withRoomSession(context, "wbstream") { session ->
                // This session crosses directly into the in-memory Go binding;
                // it is never merged into RuntimeConfigStore or TokenStore.
                call(
                    "startTransportWithLocalSession",
