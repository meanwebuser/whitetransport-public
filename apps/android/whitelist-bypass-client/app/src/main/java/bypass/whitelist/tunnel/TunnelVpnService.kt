...
class TunnelVpnService : VpnService() {
    ...
    private fun start() {
        ...
        val dnsServers = when (Prefs.dnsMode) {
            DnsMode.SYSTEM -> {
                val systemDns = getSystemDnsServers()
                systemDns.ifEmpty { listOf(Vpn.DNS_PRIMARY, Vpn.DNS_SECONDARY) }
            }
            DnsMode.CUSTOM -> {
                ...
                listOf(primary, secondary)
                    ...
                    .ifEmpty { listOf(Vpn.DNS_PRIMARY, Vpn.DNS_SECONDARY) }
            }
        }
        dnsServers.forEach(builder::addDnsServer)
        buildDirectDnsBypassRoutes(dnsServers).forEach { route ->
            builder.excludeRoute(IpPrefix(InetAddress.getByName(route.address), route.prefixLength))
        }
        TunnelServiceState.logCallback?.invoke("VPN builder: DNS resolves via underlying network")
...
internal data class DirectDnsBypassRoute(
    val address: String,
    val prefixLength: Int,
)

internal fun buildDirectDnsBypassRoutes(dnsServers: List<String>): List<DirectDnsBypassRoute> =
    dnsServers.map { rawAddress ->
        val address = rawAddress.trim()
        val prefixLength = if (InetAddress.getByName(address) is Inet4Address) 32 else 128
        DirectDnsBypassRoute(address, prefixLength)
    }.distinct()
