...
LOG="${LOG:-/tmp/wt-android-go-runtime-e2e-$(date +%Y%m%d_%H%M%S).log}"
...
timeout --foreground --signal=TERM --kill-after=10s "${INSTRUMENTATION_TIMEOUT_SECONDS}s" \
  "${ADB_BASE[@]}" shell am instrument -w -r "${ARGS[@]}" bypass.whitelist.test/androidx.test.runner.AndroidJUnitRunner \
