...
elif [[ " $* " == *" shell am get-current-user "* ]]; then
  printf '0\n'
...
ADB="$BIN/adb" GRADLE="$BIN/gradle" DEVICE=emulator-5554 \
  ...
  WT_PROBE_EXPECTED="Example Domain" \
...
grep -Fq "shell 'am' 'instrument' '--user' '0' '-w' '-r'" "$FAKE_ADB_LOG"
grep -Fq "'Example Domain'" "$FAKE_ADB_LOG"
