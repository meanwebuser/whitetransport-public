...
write_result() {
  WT_ACCEPTANCE_RESULT="$RESULT" WT_ACCEPTANCE_APK="$APK" WT_ACCEPTANCE_DEVICE="$DEVICE" \
    ...
    WT_ACCEPTANCE_STARTED_AT="$STARTED_AT" WT_ACCEPTANCE_AGENT="$AGENT_DEVICE_PASSED" \
...
finish() {
  ...
  if ! "${ADB_BASE[@]}" shell run-as "$PKG" cat cache/relay.log >"$PHONE_LOG_RAW" 2>&1 || [[ ! -s "$PHONE_LOG_RAW" ]]; then
    "${ADB_BASE[@]}" logcat -d -v threadtime >"$PHONE_LOG_RAW" 2>&1 || true
  fi
...
run_agent_fast_ui() {
  local connect_ref
  ANDROID_SERIAL="$DEVICE" AGENT_DEVICE_SESSION="$AGENT_SESSION" \
    "$AGENT_DEVICE" snapshot -i --platform android >"$AGENT_SNAPSHOT_RAW" 2>&1 || fail "agent_device_fast_snapshot_failed"
  ...
  if [[ -z "$connect_ref" ]]; then
    ...
    ANDROID_SERIAL="$DEVICE" AGENT_DEVICE_SESSION="$AGENT_SESSION" \
      "$AGENT_DEVICE" snapshot -i --force-full --platform android >"$AGENT_SNAPSHOT_RAW" 2>&1 || fail "agent_device_fast_snapshot_after_consent_failed"
    connect_ref="$(sed -n 's/.*\(@e[0-9][0-9]*\) \[togglebutton\] "\(Подключиться\|Connect\)".*/\1/p' "$AGENT_SNAPSHOT_RAW" | head -n 1)"
  fi
  ...
  if [[ "$VPN_CONSENT_ACCEPTED" == true ]]; then
    ANDROID_SERIAL="$DEVICE" AGENT_DEVICE_SESSION="$AGENT_SESSION" \
      "$AGENT_DEVICE" snapshot -i --force-full --platform android >"$VPN_CONSENT_AFTER_RAW" 2>&1 || fail "vpn_consent_after_snapshot_failed"
  ...
  ANDROID_SERIAL="$DEVICE" AGENT_DEVICE_SESSION="$AGENT_SESSION" \
    "$AGENT_DEVICE" screenshot "$FAST_SCREENSHOT" --platform android >/dev/null 2>&1 || fail "fast_ui_screenshot_failed"
...
"${ADB_BASE[@]}" shell input keyevent 26 >/dev/null 2>&1 || true
...
ANDROID_SERIAL="$DEVICE" AGENT_DEVICE_SESSION="$AGENT_SESSION" \
  "$AGENT_DEVICE" snapshot -i --platform android >"$AGENT_SNAPSHOT_RAW" 2>&1 || fail "agent_device_snapshot_failed"
ANDROID_SERIAL="$DEVICE" AGENT_DEVICE_SESSION="$AGENT_SESSION" \
  "$AGENT_DEVICE" screenshot "$AGENT_SCREENSHOT" --platform android >/dev/null 2>&1 || fail "agent_device_screenshot_failed"
