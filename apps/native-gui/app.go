...
func newApp(service runtimeService, resources guiruntime.RuntimeResourceSummary, logSink *guiruntime.LogSink, supervisor daemonSupervisor) (*App, error) {
	...
	return &App{service: service, logs: logSink, resources: resources, supervisor: supervisor, systemVPN: systemVPN}, nil
}
...
func canReplaceActiveSystemVPNProfile(active, next systemVPNProfileIdentity) bool {
	if !active.Ready || !next.Ready || active.DaemonInstanceID != next.DaemonInstanceID || active.SessionID != next.SessionID {
		return false
	}
	if active.SelectedNodeID != next.SelectedNodeID {
		return false
	}
...
func (a *App) replaceActiveSystemVPNProfile(status guiruntime.DesktopStatus, identity systemVPNProfileIdentity) (guiruntime.DesktopStatus, error) {
	...
	if stopErr != nil {
		...
		a.activeSystemVPNProfile = systemVPNProfileIdentity{}
		return applySystemVPNObservation(status, systemVPNObservation{State: guiruntime.SystemVPNError, ProviderState: guiruntime.SystemVPNError}), errors.Join(stopErr, rollbackErr)
	}
	...
	if startErr != nil {
		...
		_, rollbackErr := a.service.Disconnect(a.context())
		a.activeSystemVPNProfile = systemVPNProfileIdentity{}
		return applySystemVPNObservation(status, systemVPNObservation{State: guiruntime.SystemVPNError, ProviderState: guiruntime.SystemVPNError}), errors.Join(startErr, cleanupErr, rollbackErr)
	}
