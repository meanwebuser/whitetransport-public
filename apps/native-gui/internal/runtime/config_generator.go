...
func buildCarrierConfigs(tokenStore tokenStoreFile) ([]string, []json.RawMessage, error) {
	...
	for _, binding := range enabledBindings {
		...
		case binding.Platform == "wbstream" && binding.ConnectionType == "vp8":
			...
			// DION and Telemost are intentionally not projected into the managed
			// client until their provider-specific authoritative origins are part of
			// the config contract; an unresolved fallback would invalidate the whole
			// system-VPN profile even when WBStream is the active egress.
		}
	}
