...
func TestServiceListServersSortsAvailableServersByLabel(t *testing.T) {
	t.Parallel()

	service := newTestService(t, &stubClient{
		nodes: []runtimeapi.Node{
			{NodeID: "example-runtime-canary", Label: "example-runtime-canary", Available: true},
			{NodeID: "offline-node", Label: "alpha-offline", Available: false},
			{NodeID: "primary-test-node", Label: "primary-test-node", Available: true},
		},
	})
	...
	wantIDs := []string{"example-runtime-canary", "primary-test-node", "offline-node"}
