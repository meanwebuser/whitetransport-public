...
func CurrentRuntimeMode() string {
	...
	if err == nil {
		...
		for _, name := range []string{"whitetransportd", "whitetransportd.exe"} {
			if info, err := os.Stat(filepath.Join(execDir, name)); err == nil && info.Mode().Perm()&0o111 != 0 {
				return ModeManaged
			}
		}
	}
...
func (r ResourceResolver) fileCandidates(kind string, required bool, requireExecutable bool, candidates []pathCandidate) []RuntimeResourceCandidate {
	...
	for _, candidate := range candidates {
		...
		seen[target] = true
		out = append(out, inspectCandidate(kind, candidate.source, target, required, requireExecutable))
	}
	return out
}
...
func daemonBinaryCandidates(r ResourceResolver, repoRoot string) []pathCandidate {
	...
	candidates = append(candidates,
		pathCandidate{source: "bundle", target: filepath.Join(execDir, "whitetransportd")},
		pathCandidate{source: "bundle:resources", target: filepath.Join(execDir, "resources", "whitetransportd")},
		pathCandidate{source: "cwd", target: filepath.Join(r.cwd, "whitetransportd")},
	)
	if repoRoot != "" {
		candidates = append(candidates,
...
func singBoxCandidates(r ResourceResolver, repoRoot string) []pathCandidate {
	...
	execDir := filepath.Dir(r.executablePath)
	candidates = append(candidates,
		pathCandidate{source: "bundle", target: filepath.Join(execDir, "sing-box")},
		pathCandidate{source: "bundle:resources", target: filepath.Join(execDir, "resources", "sing-box")},
	)
	if repoRoot != "" {
...
func inspectCandidate(kind string, source string, target string, required bool, requireExecutable bool) RuntimeResourceCandidate {
	...
	candidate.Executable = info.Mode()&0o111 != 0
