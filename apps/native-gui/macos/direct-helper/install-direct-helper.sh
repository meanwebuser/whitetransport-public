...
copy_or_build() {
  ...
  if [[ -z "$source" ]]; then
    case "$name" in
      direct-helper)
        (cd "$ROOT_DIR" && GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 "$GO_BIN" build -trimpath -o "$SIDECAR_BUILD_DIR/$name" .)
        ;;
      tun2socks)
        (cd "$ROOT_DIR/../../../../third_party/tun2socks/upstream" && GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 "$GO_BIN" build -trimpath -ldflags='-s -w' -o "$SIDECAR_BUILD_DIR/$name" .)
        ;;
    esac
