...
import { createWailsBackend, getWailsApp, hasWailsHost } from './wails-transport.ts';
import { normalizeLocalUserAuthSessionStatus, UnsupportedCapabilityError } from './transport-contract.ts';
import {
...
const WtTransport = {
  ...
  beginRoomAuth: async (): Promise<void> => {
    if (!hasCapacitor() || !CapacitorBackend.beginRoomAuth) {
      throw new UnsupportedCapabilityError('transport', currentHost());
    }
    await CapacitorBackend.beginRoomAuth();
  },
  getRoomAuthStatus: async (): Promise<boolean> => {
    if (!hasCapacitor() || !CapacitorBackend.getRoomAuthStatus) {
      throw new UnsupportedCapabilityError('transport', currentHost());
    }
    return (await CapacitorBackend.getRoomAuthStatus()).ready === true;
  },
