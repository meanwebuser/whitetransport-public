...
import { getWailsApp, hasWailsHost } from '../../native/wails-transport'
import WtTransport from '../../native/wt-transport'
...
export function SettingsScreen({
  ...
  const [roomFirstReady, setRoomFirstReady] = useState<boolean | null>(null)
  const [roomFirstMessage, setRoomFirstMessage] = useState('')
  const [desktopRoomAuthBusy, setDesktopRoomAuthBusy] = useState(false)
  const [roomAuthReady, setRoomAuthReady] = useState<boolean | null>(null)
  const [roomAuthBusy, setRoomAuthBusy] = useState(false)
  const [roomAuthError, setRoomAuthError] = useState('')
  ...
  const isDesktop = capabilities?.host === 'wails'
  const isAndroid = capabilities?.host === 'capacitor'
  ...
  useEffect(() => {
    let current = true
    if (!isDesktop || !hasWailsHost()) {
      setRoomFirstReady(null)
      setRoomFirstMessage('')
      return () => { current = false }
    }

    const app = getWailsApp()
    if (!app?.HasClientRoomCredentials) return () => { current = false }

    const refreshRoomAuthStatus = () => {
      if (!app.GetRoomAuthStatus) return
      void app.GetRoomAuthStatus().then((status) => {
        if (!current || !status.message) return
        setRoomFirstMessage(status.message)
        if (status.state === 'ready') setRoomFirstReady(true)
      })
    }

    void Promise.all([
      app.HasClientRoomCredentials(),
      app.GetClientCredentialRefreshMessage?.() ?? Promise.resolve(''),
    ]).then(([ready, message]) => {
      if (!current) return
      setRoomFirstReady(ready)
      setRoomFirstMessage(message)
    }).catch(() => {
      if (!current) return
      setRoomFirstReady(null)
      setRoomFirstMessage('Локальный runtime ещё не ответил.')
    })

    refreshRoomAuthStatus()
    const roomAuthTimer = app.GetRoomAuthStatus ? window.setInterval(refreshRoomAuthStatus, 1500) : undefined

    return () => {
      current = false
      if (roomAuthTimer !== undefined) window.clearInterval(roomAuthTimer)
    }
  }, [isDesktop])
  ...
  useEffect(() => {
    let current = true
    if (!isAndroid) {
      setRoomAuthReady(null)
      setRoomAuthError('')
      return () => { current = false }
    }
    const refresh = () => {
      void WtTransport.getRoomAuthStatus().then((ready) => {
        if (!current) return
        setRoomAuthReady(ready)
        setRoomAuthError('')
      }).catch(() => {
        if (!current) return
        setRoomAuthReady(null)
        setRoomAuthError('Локальный вход пока недоступен.')
      })
    }
    refresh()
  ...
  const beginRoomAuth = async () => {
    setRoomAuthBusy(true)
    setRoomAuthError('')
    try {
      await WtTransport.beginRoomAuth()
    } catch {
      setRoomAuthError('Не удалось открыть локальный вход.')
    } finally {
      setRoomAuthBusy(false)
    }
  }

  const beginDesktopRoomAuth = async () => {
    const app = getWailsApp()
    if (!app?.BeginRoomAuth) {
      setRoomFirstMessage('Встроенный вход пока недоступен в этой desktop-сборке.')
      return
    }
    setDesktopRoomAuthBusy(true)
    try {
      const status = await app.BeginRoomAuth()
      setRoomFirstMessage(status.message)
      setRoomFirstReady(status.state === 'ready')
    } catch {
      setRoomFirstMessage('Не удалось открыть встроенный вход.')
    } finally {
      setDesktopRoomAuthBusy(false)
    }
  }
  ...
  return (
    <section className="shell-screen shell-settings" aria-labelledby="settings-heading">
      ...
      {isDesktop ? (
        <section className="shell-settings-card" data-testid="room-first-readiness" aria-labelledby="room-first-heading">
          <div className="shell-card-heading">
            <div>
              <p className="shell-card-kicker">Room-first</p>
              <h2 id="room-first-heading">Комната создаётся на устройстве</h2>
            </div>
            <ShieldCheck aria-hidden="true" size={22} />
          </div>
          <p>
            {roomFirstReady === true
              ? 'Готово: после подключения клиент создаст комнату, а node войдёт в неё гостем.'
              : 'Встроенный логин будет подключён здесь; пока room-first ждёт локальную сессию совместимого provider.'}
          </p>
          <button className="shell-primary-button" data-testid="desktop-room-auth-button" type="button" disabled={desktopRoomAuthBusy} onClick={() => { void beginDesktopRoomAuth() }}>
            {desktopRoomAuthBusy ? 'Открываю вход…' : 'Войти в WB Stream'}
          </button>
          {roomFirstMessage ? <p className="shell-muted-text">{roomFirstMessage}</p> : null}
        </section>
      ) : null}

      {isAndroid ? (
        <section className="shell-settings-card" data-testid="room-auth-readiness" aria-labelledby="room-auth-heading">
          <div className="shell-card-heading">
            <div>
              <p className="shell-card-kicker">Room-first</p>
              <h2 id="room-auth-heading">Встроенный вход WB Stream</h2>
            </div>
            <ShieldCheck aria-hidden="true" size={22} />
          </div>
          <p>
            {roomAuthReady === true
              ? 'Готово: следующая сессия создаст комнату на этом устройстве.'
              : 'Вход открывается внутри WhiteTransport и остаётся только на этом устройстве.'}
          </p>
          <button className="shell-primary-button" data-testid="room-auth-button" type="button" disabled={roomAuthBusy} onClick={() => { void beginRoomAuth() }}>
            {roomAuthBusy ? 'Открываю вход…' : 'Войти в WB Stream'}
          </button>
          {roomAuthError ? <p className="shell-muted-text">{roomAuthError}</p> : null}
        </section>
      ) : null}
