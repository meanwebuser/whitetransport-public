...
describe('shared native client shell', () => {
  ...
  it('allows Disconnect while the native transport recovers with system VPN still active', async () => {
    const user = userEvent.setup()
    native.transport.getStatus.mockResolvedValue({ ...native.status, status: 'reconnecting', state: 'reconnecting', active: false, transportState: 'connecting', systemVpnState: 'connected' })
    native.transport.getConnectionState.mockResolvedValue({
      ...native.connectionState,
    ...
    renderShell()
    expect(await screen.findByTestId('connection-status')).toHaveTextContent(/подключение/i)
    const button = screen.getByTestId('power-control')
    expect(button).toHaveAccessibleName('Отключить')
